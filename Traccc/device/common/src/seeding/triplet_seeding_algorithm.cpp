...
auto triplet_seeding_algorithm::operator()(
  ...
  vecmem::data::vector_buffer<unsigned int> grid_capacities_buffer(grid_bins,
                                                                   mr().main);
  copy().setup(vecmem::no_event, grid_capacities_buffer);
  copy().memset(vecmem::no_event, grid_capacities_buffer, 0);
  ...
  copy().setup(vecmem::no_event, grid_buffer._buffer);
  ...
  copy().setup(vecmem::no_event, grid_prefix_sum_buffer);
  ...
  device::doublet_counter_collection_types::buffer doublet_counter_buffer{
      n_spacepoints, mr().main, vecmem::data::buffer_type::resizable};
  copy().setup(vecmem::no_event, doublet_counter_buffer);
  ...
  triplet_counter_spM_collection_types::buffer triplet_counter_spM_buffer{
      n_doublets, mr().main};
  copy().setup(vecmem::no_event, triplet_counter_spM_buffer);
  copy().memset(vecmem::no_event, triplet_counter_spM_buffer, 0);
  triplet_counter_collection_types::buffer triplet_counter_midBot_buffer{
      globalCounter_host->m_nMidBot, mr().main,
      vecmem::data::buffer_type::resizable};
  copy().setup(vecmem::no_event, triplet_counter_midBot_buffer);
  ...
  copy().setup(vecmem::no_event, triplet_buffer);
  ...
  edm::seed_collection::buffer seed_buffer(
      globalCounter_host->m_nTriplets, mr().main,
      vecmem::data::buffer_type::resizable);
  copy().setup(vecmem::no_event, seed_buffer);
