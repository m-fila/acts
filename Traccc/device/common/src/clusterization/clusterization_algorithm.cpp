...
clusterization_algorithm::execute_impl(
  ...
  copy().setup(vecmem::no_event, measurements);
  ...
  if (m_config.sort_cells) {
    ...
    copy().setup(vecmem::no_event, *sorted_cells);
    ...
    copy().setup(vecmem::no_event, permutation_map_buffer);
  ...
  if (keep_disjoint_set) {
    ...
    copy().setup(vecmem::no_event, *cluster_data);
