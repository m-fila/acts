...
auto gbts_seeding_algorithm::make_nodes(
  ...
  vecmem::data::vector_buffer<unsigned int> layerCounts_buf(cfg.nLayers + 1,
                                                            mr().main);
  copy().memset(vecmem::no_event, layerCounts_buf, 0);

  vecmem::data::vector_buffer<float4> reducedSP_buf(nSp, mr().main);
  copy().setup(vecmem::no_event, reducedSP_buf);

  vecmem::data::vector_buffer<unsigned short> spacepointsLayer_buf(nSp,
                                                                   mr().main);
  copy().setup(vecmem::no_event, spacepointsLayer_buf);

  vecmem::data::vector_buffer<short> volumeToLayerMap_buf(
      static_cast<unsigned int>(cfg.volumeToLayerMap.size()), mr().main);
  copy().setup(vecmem::no_event, volumeToLayerMap_buf);
  copy()(vecmem::no_event, vecmem::get_data(cfg.volumeToLayerMap),
  ...
  if (!cfg.surfaceToLayerMap.empty()) {
    ...
    copy().setup(vecmem::no_event, surfaceToLayerMap_buf);
    copy()(vecmem::no_event, vecmem::get_data(cfg.surfaceToLayerMap),
           surfaceToLayerMap_buf);
  }

  vecmem::data::vector_buffer<char> layerType_buf(cfg.nLayers, mr().main);
  copy().setup(vecmem::no_event, layerType_buf);
  copy()(vecmem::no_event, vecmem::get_data(cfg.layerInfo.type), layerType_buf);
  ...
  copy().setup(vecmem::no_event, sp_params_buf);
  vecmem::data::vector_buffer<unsigned int> original_sp_idx_buf(nSp, mr().main);
  copy().setup(vecmem::no_event, original_sp_idx_buf);
  ...
  copy().setup(vecmem::no_event, layer_info_buf);
  copy()(vecmem::no_event, vecmem::get_data(cfg.layerInfo.info),
         layer_info_buf);

  vecmem::data::vector_buffer<std::pair<float, float>> layer_geo_buf(
      cfg.nLayers, mr().main);
  copy().setup(vecmem::no_event, layer_geo_buf);
  copy()(vecmem::no_event, vecmem::get_data(cfg.layerInfo.geo), layer_geo_buf);

  vecmem::data::vector_buffer<unsigned int> node_phi_index_buf(nNodes,
                                                               mr().main);
  copy().setup(vecmem::no_event, node_phi_index_buf);

  vecmem::data::vector_buffer<unsigned int> node_eta_index_buf(nNodes,
                                                               mr().main);
  copy().setup(vecmem::no_event, node_eta_index_buf);
  ...
  vecmem::data::vector_buffer<unsigned int> eta_phi_histo_buf(hist_size,
                                                              mr().main);
  copy().setup(vecmem::no_event, eta_phi_histo_buf);
  copy().memset(vecmem::no_event, eta_phi_histo_buf, 0);
  vecmem::data::vector_buffer<unsigned int> phi_cusums_buf(hist_size,
                                                           mr().main);
  copy().setup(vecmem::no_event, phi_cusums_buf);
  ...
  vecmem::data::vector_buffer<unsigned int> eta_node_counter_buf(cfg.n_eta_bins,
                                                                 mr().main);
  copy().setup(vecmem::no_event, eta_node_counter_buf);
  ...
  copy().setup(vecmem::no_event, tau_lut_buf);
  if (!cfg.tau_lut.empty()) {
    copy()(vecmem::no_event, vecmem::get_data(cfg.tau_lut), tau_lut_buf);
  }
  ...
  copy().setup(vecmem::no_event, eta_bin_views_buf);
  copy()(vecmem::no_event, vecmem::get_data(eta_bin_views), eta_bin_views_buf);

  vecmem::data::vector_buffer<float> bin_rads_buf(2 * cfg.n_eta_bins,
                                                  mr().main);
  copy().setup(vecmem::no_event, bin_rads_buf);
...
auto gbts_seeding_algorithm::create_edges(
  ...
  copy().setup(vecmem::no_event, bin_pair_views_buf);
  copy()(vecmem::no_event, vecmem::get_data(bin_pair_views),
         bin_pair_views_buf);

  vecmem::data::vector_buffer<float> bin_pair_dphi_buf(nUsedBinPairs,
                                                       mr().main);
  copy().setup(vecmem::no_event, bin_pair_dphi_buf);
  copy()(vecmem::no_event, vecmem::get_data(bin_pair_dphi), bin_pair_dphi_buf);
  ...
  copy().setup(vecmem::no_event, edge_params_buf);
  vecmem::data::vector_buffer<uint2> edge_nodes_buf(nMaxEdges, mr().main);
  copy().setup(vecmem::no_event, edge_nodes_buf);
  vecmem::data::vector_buffer<unsigned int> num_incoming_edges_buf(nNodes + 1,
                                                                   mr().main);
  copy().setup(vecmem::no_event, num_incoming_edges_buf);
  copy().memset(vecmem::no_event, num_incoming_edges_buf, 0);
  ...
  copy().setup(vecmem::no_event, edge_links_buf);
  ...
  vecmem::data::vector_buffer<unsigned char> num_neighbours_buf(nEdges,
                                                                mr().main);
  copy().setup(vecmem::no_event, num_neighbours_buf);
  copy().memset(vecmem::no_event, num_neighbours_buf, 0);

  vecmem::data::vector_buffer<int> reIndexer_buf(nEdges, mr().main);
  copy().setup(vecmem::no_event, reIndexer_buf);
  // Byte-fill 0xFF -> int -1, the "edge not kept" sentinel checked by
  // gbts_reindex_edges / gbts_compress_graph.
  copy().memset(vecmem::no_event, reIndexer_buf, 0xFF);

  vecmem::data::vector_buffer<unsigned int> neighbours_buf(
      cfg.max_num_neighbours * nEdges, mr().main);
  copy().setup(vecmem::no_event, neighbours_buf);
  copy().memset(vecmem::no_event, neighbours_buf, 0);
  ...
  copy().setup(vecmem::no_event, output_graph_buf);
...
auto gbts_seeding_algorithm::extract_seeds(
  ...
  vecmem::data::vector_buffer<int2> path_store_buf(nPaths + nTerminusEdges,
                                                   mr().main);
  copy().setup(vecmem::no_event, path_store_buf);
  vecmem::data::vector_buffer<int2> seed_proposals_buf(nPaths, mr().main);
  copy().setup(vecmem::no_event, seed_proposals_buf);
  vecmem::data::vector_buffer<char> seed_ambiguity_buf(nPaths, mr().main);
  copy().setup(vecmem::no_event, seed_ambiguity_buf);

  vecmem::data::vector_buffer<unsigned long long int> edge_bids_buf(
      nConnectedEdges, mr().main);
  copy().setup(vecmem::no_event, edge_bids_buf);
  copy().memset(vecmem::no_event, edge_bids_buf, 0);
  ...
  copy().setup(vecmem::no_event, output_seeds);

  vecmem::data::vector_buffer<unsigned long long int> hit_bids_buf(nSp,
                                                                   mr().main);
  copy().setup(vecmem::no_event, hit_bids_buf);
  copy().memset(vecmem::no_event, hit_bids_buf, 0);
...
auto gbts_seeding_algorithm::operator()(
  ...
  copy().setup(vecmem::no_event, counters_buf);
  copy().memset(vecmem::no_event, counters_buf, 0);
